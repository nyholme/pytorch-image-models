...
#def cnsteerablecnn(pretrained: bool = False, model_cfg: Optional[Any] = None, **kwargs) -> CNSteerableCNN:
@register_model
...
class CNSteerableCNN(torch.nn.Module):
    def __init__(
        ...
        self.feat_types = [nn.FieldType(self.r2_act,  1*[self.r2_act.trivial_repr])]
        for n_reps in n_rep_channels:
            self.feat_types.append(nn.FieldType(self.r2_act,  n_reps*[self.r2_act.regular_repr]))
        ...
        dense_input_dim = n_rep_channels[-1] * (28 - np.sum(np.array(kernel_sizes) - np.array(len(kernel_sizes) * [3])))**2
    ...
    def forward(self, x):
        ...
        #print(f"shape before conv_blocks: {x.shape}")
        for conv_block in self.conv_blocks:
            x = conv_block(x)
        ...
        # pool over the spatial dimensions
        x = self.spatial_pool(x)
        #print(f"shape after spatial_pool: {x.shape}")
        
        # pool over the group
        x = self.group_pool(x)
        #print(f"shape after group_pool: {x.shape}")
        # at this point the channels should still be rots of eachother (up to rounding errors)
        #print(x)

        #print("geometric tensor shape", x.shape)
        x = x.tensor
