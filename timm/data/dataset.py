...
# see https://www.c3se.chalmers.se/documentation/software/machine_learning/datasets/#using-this-dataset
class ImageNetFromZip(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, int]:
        ...
        img = torch.from_numpy(np.asarray(Image.open(io.BytesIO(self.zf.read(imgpath)))))
        #print(f"Image shape before: {img.shape}")
        #img = torch.movedim(self.resize(torch.movedim(img, -1, 0)), 0, -1)
        img = self.resize(torch.movedim(img, -1, 0))
